          ...
          <div key={link.id} className="card p-0 overflow-hidden">
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <a
                  ...
                  className="text-xs text-brand-500 hover:underline truncate block max-w-lg"
              ...
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-xs text-gray-400">
                  {link.artikelen.length} {link.artikelen.length === 1 ? "artikel" : "artikelen"}
                </span>
                <span className="text-gray-300 text-sm">{uitgeklapt === link.id ? "▲" : "▼"}</span>
                <button
                  onClick={(e) => { e.stopPropagation(); handleVerwijder(link.id); }}
            ...
            {uitgeklapt === link.id && (
              <div className="border-t border-gray-100 bg-gray-50/50 p-4 space-y-4">
                <div>
                  <h4 className="text-sm font-semibold text-[#0f172a] mb-2">Gekoppelde artikelen op TravelTribe.life</h4>
                    ...
                    <div className="space-y-2 mb-3">
                      {link.artikelen.map((artikel) => (
                        <div key={artikel.id} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2 border border-gray-100">
                          ...
                          <button
                            onClick={() => handleArtikelVerwijder(artikel.id)}
                  ...
                  {artikelFormulier.linkId === link.id ? (
                    <form onSubmit={handleArtikelToevoegen} className="flex gap-2 items-end">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          placeholder="bijv. De beste hotels in Barcelona"
                          value={artikelFormulier.titel}
                          onChange={(e) => setArtikelFormulier((p) => ({ ...p, titel: e.target.value }))}
                          required
                          autoFocus
                        />
                      </div>
                      <div className="flex-1">
                        ...
                        <input
                          type="url"
                          className="input text-sm py-2"
                          placeholder="https://traveltribe.life/..."
                          value={artikelFormulier.url}
                          onChange={(e) => setArtikelFormulier((p) => ({ ...p, url: e.target.value }))}
                          required
                        />
                      </div>
                    ...
                    <button
                      onClick={() => setArtikelFormulier({ linkId: link.id, ...leegArtikel })}
